...
*/
  ...
  /** Sink：*_url(...) 调用本体；以及 link_to(..., url, ...) 的第 2 个位置参数 */
  predicate isSink(DataFlow::Node sink) {
    // 1) *_url(...) —— 方法调用本身
    exists(MethodCall m |
      m.getMethodName().regexpMatch(".*_url$") and
      sink.asExpr().getExpr() = m
    )
    or
    // 2) link_to(label, url, ...) —— 第二个位置参数（index 1）
    exists(MethodCall m |
      m.getMethodName() = "link_to" and
      sink.asExpr().getExpr() = m.getArgument(1)
    )
    or
    // 3) Rodauth: reset_password_email_link(...) 这类隐式构造绝对链接的方法
    exists(MethodCall m |
      m.getMethodName() = "reset_password_email_link" and
      sink.asExpr().getExpr() = m
    )
    or
    // 4) URL: URI.join(host, *_path(...)) / Addressable::URI.join(...)
    exists(MethodCall m |
      m.getMethodName() = "join" and
      m.getReceiver().toString().regexpMatch("^URI$|^Addressable::URI$") and
      exists(MethodCall p |
        p = m.getAnArgument() and p.getMethodName().regexpMatch(".*_path$")
      ) and
      sink.asExpr().getExpr() = m
    )    
    or
    // 额外防护纯手拼URL: 任意表达式文本里直接出现 http(s)://（覆盖普通/插值字符串）
    // 说明：Ruby QL 没有 InterpolatedString，这里用 Expr + toString() 做通配
    exists(Expr s |
      s.toString().regexpMatch("http[s]?://") and
      sink.asExpr().getExpr() = s
    )
    or
    // 额外防护纯手拼URL: 字符串拼接/追加：+, <<（很多项目手拼 host + *_path）
    exists(BinaryOperation op |
      op.getOperator().regexpMatch("^\\+|<<$") and
      op.toString().regexpMatch("http[s]?://") and
      sink.asExpr().getExpr() = op
    )
  // ★ ADD: 任何看起来像 URL 的表达式都当作 sink（覆盖普通/插值/拼接）
  or exists(Expr e |
    sink.asExpr().getExpr() = e and
    e.toString().regexpMatch("(?i)\\b(?:https?|ftp)://")
  )
